...
modele <- aov( Température ~mois ,prevision)
...
kruskal.test(Température ~mois ,prevision)
prevision$mois <- NULL

```
La différence est très significative.

Il est donc conseillé de mieux se couvrir en hiver au vu de la différence très significative de température.
...
prevision$Temperature_Jour_Suivant <- Lag(prevision$Température, shift = -2)
#on supprimme la derniere observation
library(dplyr)
prevision = slice(prevision, 1:(n() -2))
...
```
Remarque importante : les lignes ne sont plus ordonnés chronologiquement.
...
Cette hypothèse se confirmera lorsque nous regarderons l'histogramme des résidus.
...
Un R² de 0.57 est trouvé et le R²ajusté = 0.57 également. 
...
D'où 57% de la variance est expliqué par le modèle de regréssion.
...
On trouve un R² de 0.53 et le R² ajusté également.
...
<br> </br>
TempÃ.rature.minimale.du.sol.sur.12.heures  a la F value la plus forte, sa pvalue permet de rejeter H0
...
add1(update(reg1,~.+Température.minimale.du.sol.sur.12.heures),
...
```
<br> </br>
On introduit les variables jusqu'à que aucune n'ai une augmentation du R significative lors de son introduction dans le modèle.
...
```{r}
reg.mul.forward <- lm(Temperature_Jour_Suivant~Température.minimale.du.sol.sur.12.heures+
            Température +
              Rafales.sur.une.période  +
              Hauteur.de.base.2  +
              Pression.station  +
...
On a un R² de 0.57 et un R² également. 
...
```
à chaque itération, on retire la p-value de F la plus faible
```{r,results='hide' ,eval=FALSE, include=FALSE}
drop1(update(lm.full,~.-Vitesse.du.vent.moyen.10.mn
             ), test="F")
```

```{r,results='hide',eval=FALSE, include=FALSE}
drop1(update(lm.full,~.-Vitesse.du.vent.moyen.10.mn-Température
             ), test="F")
```

```{r,results='hide',eval=FALSE, include=FALSE}
drop1(update(lm.full,~.-Vitesse.du.vent.moyen.10.mn-Température-Hauteur.de.base.2 
             ), test="F")
```

```{r,results='hide',eval=FALSE, include=FALSE}
drop1(update(lm.full,~.-Vitesse.du.vent.moyen.10.mn-Température-Hauteur.de.base.2-Variation.de.pression.en.3.heures
             ), test="F")
```

```{r,results='hide',eval=FALSE, include=FALSE}
drop1(update(lm.full,~.-Vitesse.du.vent.moyen.10.mn-Température-Hauteur.de.base.2-Variation.de.pression.en.3.heures-Précipitations.dans.les.24.dernières.heures
             ), test="F")
```

```{r,results='hide',eval=FALSE, include=FALSE}
drop1(update(lm.full,~.-Vitesse.du.vent.moyen.10.mn-Température-Hauteur.de.base.2-Variation.de.pression.en.3.heures-Précipitations.dans.les.24.dernières.heures-Direction.du.vent.moyen.10.mn 
             ), test="F")
```

```{r,results='hide',eval=FALSE, include=FALSE}
drop1(update(lm.full,~.-Vitesse.du.vent.moyen.10.mn-Température-Hauteur.de.base.2-Variation.de.pression.en.3.heures-Précipitations.dans.les.24.dernières.heures-Direction.du.vent.moyen.10.mn-Précipitations.dans.les.3.dernières.heures
             ), test="F")
```

```{r,results='hide',eval=FALSE, include=FALSE}

drop1(update(lm.full,~.-Vitesse.du.vent.moyen.10.mn-Température-Hauteur.de.base.2-Variation.de.pression.en.3.heures-Précipitations.dans.les.24.dernières.heures-Direction.du.vent.moyen.10.mn-Précipitations.dans.les.3.dernières.heures-Rafale.sur.les.10.dernières.minutes
             ), test="F")
...
```{r,results='hide',eval=FALSE, include=FALSE}
drop1(update(lm.full,~.-Vitesse.du.vent.moyen.10.mn-Température-Hauteur.de.base.2-Variation.de.pression.en.3.heures-Précipitations.dans.les.24.dernières.heures-Direction.du.vent.moyen.10.mn-Précipitations.dans.les.3.dernières.heures-Rafale.sur.les.10.dernières.minutes-Visibilité.horizontale-Pression.station     
             ), test="F")
```

```{r,results='hide',eval=FALSE, include=FALSE}
drop1(update(lm.full,~.-Vitesse.du.vent.moyen.10.mn-Température-Hauteur.de.base.2-Variation.de.pression.en.3.heures-Précipitations.dans.les.24.dernières.heures-Direction.du.vent.moyen.10.mn-Précipitations.dans.les.3.dernières.heures-Rafale.sur.les.10.dernières.minutes-Visibilité.horizontale-Pression.station-Précipitations.dans.les.6.dernières.heures 
             ), test="F")
```
...
summary(update(lm.full,~.-Vitesse.du.vent.moyen.10.mn
               ...
               -Précipitations.dans.les.3.dernières.heures
               -Rafale.sur.les.10.dernières.minutes
               -Visibilité.horizontale-Pression.station-Précipitations.dans.les.6.dernières.heures
               -Précipitations.dans.les.12.dernières.heures   ))


shapiro.test(update(lm.full,~.
                    -Vitesse.du.vent.moyen.10.mn
                    -Température-Hauteur.de.base.2
                    -Variation.de.pression.en.3.heures
                    -Précipitations.dans.les.24.dernières.heures
                    -Direction.du.vent.moyen.10.mn
                    -Précipitations.dans.les.3.dernières.heures
                    -Rafale.sur.les.10.dernières.minutes-Visibilité.horizontale
                    -Pression.station-Précipitations.dans.les.6.dernières.heures
                    -Précipitations.dans.les.12.dernières.heures   )$residuals)
...
Avec cette méthode on a un R² de 0.57, c'est également le cas du R² ajusté.
...
On obtient un R² de 0.57 et un R² de même valeur.
...
```
<br> </br>
Utulisation d'une autre librairie pour réaliser une prédiction de la régression Ridge.
```{r}
library(glmnet)


ridge_opti = glmnet(x = prevision[,names(prevision) != 'Temperature_Jour_Suivant'], 
...
* 0.57 : Régression avec toutes les variables 

* 0.53 : Régression avec variables significatives

* 0.57 : Régression forward

* 0.57 : Régression backward

* 0.57 : Régression sur ACP
...
La régression avec toutes les variables obtient un R² de 0.57, lorsque nous essayons d'obtenir une régression "presque aussi bien" en retirons les variables les moins explicatives, le R² chute à 0.53.
