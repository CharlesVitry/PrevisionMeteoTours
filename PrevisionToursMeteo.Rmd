...
On ne garde que une observation par jour, 
en effet le nbre d'observation par jour est irrégulier.
```{r}

data <- do.call(rbind, by(data, list(data$DateEnJour), 
...
Cependant nous voulons prédire la température,
nous creons une colonne température mais décaler de 2 jours
...
dim(data)
```
1655 observations de 23 variables, cela est suffisant pour la régression multiple
