...
dim(prevision)
```
1655 *0.8(=1324) observations de 21 variables,
...
Un R² de 0.67 est trouvé.
D'où 67% de la variance est expliqué par la régression.
...
<br> </br>
Représentation des résidus, on ne doit pas dépasser 5%.
```{r}
...
summary(update(lm.full,~.-Humidité
             -Précipitations.dans.les.3.dernières.heures
             -Précipitations.dans.les.6.dernières.heures
             -Précipitations.dans.les.12.dernières.heures
             -Rafale.sur.les.10.dernières.minutes
...
shapiro.test(update(lm.full,~.-Humidité
             -Précipitations.dans.les.3.dernières.heures
             -Précipitations.dans.les.6.dernières.heures
             -Précipitations.dans.les.12.dernières.heures
             -Rafale.sur.les.10.dernières.minutes
             ...
             -Pression.au.niveau.mer
             -Direction.du.vent.moyen.10.mn
             -Visibilité.horizontale   )$residuals)
...
Pour les comparaisons qui suivent, nous utilisons nos différents modèles prédictifs sur le jeu d'entrainement (1324 obs) et sur le jeu de test (331 obs).

<br> </br>
Régression avec toutes variables
```{r,warning=FALSE}
library(ROCR)
library(pROC)
#prediction
...
```
<br> </br>
Regression avec les plus significatives
```{r,warning=FALSE}
library(ROCR)
library(pROC)
#prediction
PredictEntrainement = predict(reg.mulfin, newdata = prevision)
PredictTesting = predict(reg.mulfin, newdata = testing)
...
library(pROC)
#prediction
PredictEntrainement = predict(reg.mulSansC, newdata = prevision)
PredictTesting = predict(reg.mulSansC, newdata = testing)
...
library(pROC)
#prediction
PredictEntrainement = predict(reg.mul.forward, newdata = prevision)
PredictTesting = predict(reg.mul.forward, newdata = testing)
...
library(pROC)
#prediction
PredictEntrainement = predict(reg.mul.backward, newdata = prevision)
PredictTesting = predict(reg.mul.backward, newdata = testing)
...
# library(pROC)
# #prediction
# PredictEntrainement = predict(ridge_opti, newx = prevision[,names(prevision) != 'Temperature_Jour_Suivant'])
# PredictTesting = predict(ridge_opti, newdata = testing)
# 
# #jeu d'entrainement
# auc(prevision$Temperature_Jour_Suivant, PredictEntrainement)
# 
...
<br> </br>
Toutes nos prédictions de la température du jour suivant sont parfaites !
