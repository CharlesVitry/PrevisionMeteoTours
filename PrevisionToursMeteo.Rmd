...
#data2$Type.nuage.4 <- NULL
```
Test des régressions sur cet ensemble : plus mauvais que l'autre jeu de données.
...
le premier avec 5661 observations , le second avec une observation par jour dans le but de déterminer la précipation du jour à venir.
...
Testons avec le jeu de prévision.
```{r}
...
L'observation suivante dépend d'une execution à l'autre du code (selon l'échantillonage) : 
...
```
Mais le test sur les résidus est toujours mauvais.
...
```
Le résultat est toujours mauvais.
...
#novariables$Temperature_Jour_Suivant <- prevision$Temperature_Jour_Suivant
...
Forward regression 
```{r}
#lm.full <- lm(Temperature_Jour_Suivant ~.,novariables)
```
